...
# this function takes in the ann_path and convert the csv file to dictionary, the csv file is having data
# like fileid, folder, singer, label .. etc, so we are just converting it to dic
def convert_csv_dict(anno_path, dataset_type):
    inputs_list = pandas.read_csv(anno_path)
    if dataset_type == 'train' and len(inputs_list) > 2390:
        broken_data = [2390]
        inputs_list.drop(broken_data, inplace=True)
    inputs_list = (inputs_list.to_dict()['id|folder|signer|annotation'].values())
    info_dict = dict()
    # anno_path = dataset/phoenix-2014-multisigner/annotations/manual/test.corpus.csv
...
# this function records the total gloss
...
if __name__ == '__main__':
    ...
    for md in mode:
        print('doing this for '+md+' mode')
        # generate information dict
        information = convert_csv_dict(f"{args.dataset_root}/{args.annotation_prefix.format(md)}", dataset_type=md)
        np.save(f"./{args.dataset}/{md}_info.npy", information)
        print('\n\n\nthis is the dict for '+ md + '\n\n')
        print(information)
